...
**Current status:** Track B implementation and npm/local post-publish validation are complete as of 2026-04-28. `locus-memory@3.5.3` is published to npm, one-command install is validated from the registry in `$CODEX_HOME`, and the local Codex config has been migrated to the published package runtime. `v3.5.1` adds safe MCP `cwd = "$CODEX_HOME"` startup behavior; `v3.5.2` fixes `doctor codex` ownership detection for the real `codex mcp get locus` path. `v3.5.3` is the final GitHub release candidate with README, release notes, tests, and GitHub Pages aligned to the public one-command install story.
