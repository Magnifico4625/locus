...
| One-command npm install | Local pre-publish checks complete | `locus-memory install codex`, `doctor codex`, and `uninstall codex` are implemented with skill install, lock handling, idempotency, redacted defaults, and package-runtime config generation | Registry-hosted `locus-memory@3.5.0` still needs B10 post-publish validation before claiming npm install is publicly proven |
