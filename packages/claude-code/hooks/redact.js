// Locus hook-level secret redaction (plain JS port of core/src/security/redact.ts)
// Applied BEFORE writing inbox events to disk — first defense layer.
// Must stay in sync with core REDACT_PATTERNS (Contract 1 — Security Layer 2).
//
...
/**
 * Redacts secrets from text using the same patterns as core redact.ts.
 * @param {string} text
 * @returns {string}
 */
export function redact(text) {
  ...
  for (const { pattern, replacement } of REDACT_PATTERNS) {
    // Reset lastIndex before each replace to avoid /g state leakage
    pattern.lastIndex = 0;
