...
describe('codex hook config generation', () => {
  it('generates lifecycle hooks for safe recall freshness triggers', () => {
    const config = buildCodexHooksConfig({ version: '3.6.0' });
    ...
    for (const event of ['SessionStart', 'UserPromptSubmit', 'Stop'] as const) {
      ...
      expect(config.hooks[event]?.[0]?.hooks[0]?.command).toContain('locus-memory@3.6.0');
      expect(config.hooks[event]?.[0]?.hooks[0]?.command).not.toContain('@latest');
    }
  });

  it('uses event-specific hook command arguments', () => {
    const config = buildCodexHooksConfig({ version: '3.6.0', platform: 'linux' });

    expect(config.hooks.SessionStart?.[0]?.hooks[0]?.command).toBe(
      'npx -y locus-memory@3.6.0 hook codex session-start',
    );
    expect(config.hooks.UserPromptSubmit?.[0]?.hooks[0]?.command).toBe(
      'npx -y locus-memory@3.6.0 hook codex user-prompt-submit',
    );
    expect(config.hooks.Stop?.[0]?.hooks[0]?.command).toBe(
      'npx -y locus-memory@3.6.0 hook codex stop',
    );
  });
  ...
  it('renders deterministic pretty JSON for hooks.json', () => {
    const json = renderCodexHooksJson(buildCodexHooksConfig({ version: '3.6.0' }));

    expect(JSON.parse(json)).toEqual(buildCodexHooksConfig({ version: '3.6.0' }));
    expect(json).toMatch(/^\{\n {2}"hooks":/u);
              ...
              hooks: [
                {
                  ...
                  statusMessage: 'mentions locus-memory@3.6.0 hook codex stop only in text',
                },
              ],
            },
          ],
